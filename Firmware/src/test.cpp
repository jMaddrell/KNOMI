...
void update_label_scan_networks_test() {
  ...
  lv_obj_add_style(label_test, &style_label_ap_config,
...
void update_label_networksID_test() {
  ...
  lv_obj_add_style(label_netID1_test, &style_label_ap_config,
                   LV_PART_MAIN); // 将样式添加到文字对象中
  ...
  lv_obj_add_style(label_netID2_test, &style_label_ap_config,
                   LV_PART_MAIN); // 将样式添加到文字对象中
  ...
  lv_obj_add_style(label_netID3_test, &style_label_ap_config,
                   LV_PART_MAIN); // 将样式添加到文字对象中
...
void update_blue_back_display() {
  ...
  lv_img_set_src(img_blue_test, &blue_test);
  lv_obj_align(img_blue_test, LV_ALIGN_CENTER, 0, 0);
}
